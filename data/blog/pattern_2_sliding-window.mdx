...
In this approach, we're aiming to find the minimum length of a window that meets a certain condition. We start by defining pointers for the front and end of the window and initialize the result to infinity. First, we move the front pointer, `i`, forward as far as possible until the condition is met, effectively expanding the window. If we reach the end without meeting the condition, we simply test the condition again and break if necessary. Next, we move the back pointer, `j`, forward as far as possible until the condition is no longer met, thereby shrinking the window. The window [j - 1, i - 1] now satisfies the condition. Its length is calculated as `i - j + 1`. We then compare this length with the current minimum result and update the result if this window is shorter.

```js
// Pseudo code
// initialize front and back pointers: i,j and final result: ans with Infinity.
// while i < n.
//    Move the front: i as far as possible until a condtion is met.
//    Test the condition again. If it fails, it means we have reached the end. //    Move the back as far as possible until the condtion is NOT met.
//    Range: [j - 1, i - 1] is the smallest window which meets the condition
//    ans = min(ans, i - 1 - (j - 1) + 1)
```

```TypeScript
function minSlidingWindow(...) {
    const n = nums.length;
    let i = 0; // right index
    let j = 0; // left index
    // let total = 0;
    let ans = Infinity;
    const state = ....
    const meetCondition = (state) => .......;
    while (i < n) {
        // move the right index i as far as possible until the condition is met
        while ((i < n) && !meetCondition(state)) {
            // Add nums[i] to sliding window
            // Update the state
            i += 1;
        }
        // If the condition is not met, there is no need to shrink the window.
        // Otherwise, whether we run to the end of the array, we still need to shrink the window.
        if (!meetCondition(state)) {
            break;
        }
        // move the left index j as far as possible unitl the condition is not met
        while (meetCondition(state)) {
            // drop nums[j] from sliding window
            // Update the state
            j += 1;
        }
        // the range [j - 1, i - 1] meets the condition and [j, i - 1] does not meet.
        // The length of [j - 1, i - 1] is i - j + 1.
...
In this approach, we're aiming to find the maxinum length of a window that meets a certain condition. We start by defining pointers for the front and end of the window and initialize the result to 0. First, we move the front pointer, `i`, forward as far as possible until the condition is NOT met, effectively expanding the window. If we reach the end without NOT meeting the condition, we simply test the condition again and compare the result with final result and break if necessary. If we reach thend with NOT meeting the condition, we compare the result with the final result. Next, we move the back pointer, `j`, forward as far as possible until the condition is no longer met, thereby shrinking the window. The window [j - 1, i - 1] now satisfies the condition. Its length is calculated as `i - j + 1`. We then compare this length with the current minimum result and update the result if this window is shorter.

```js
// Pseudo code
// initialize front and back pointers: i,j and final result: ans with 0.
// while i < n.
//    Move the front i as far as possible until a condtion is NOT met.
//    Test the condtion again.
//        If it still meets, it means we have reached the end. [j, i - 1] (i = n) meet the condition. we break out.
//        Otherwise, [j, i - 2] meets the conditons and we continue the loop.
//    Move the back j as far as possible until the condtion is met.
```
...
function maxSlidingWindow(...) {
    ...
    const meetCondition = (state) => .......;
    while (i < n) {
        // move the right index i as far as possible until the condition is NOT met or reach the end of the array
        while ((i < n) && meetCondition(state)) {
            // Add nums[i] to sliding window
            // Update the state
            i += 1;
        }
        // If the condition is still met, it means it breaks because we have reached the end of array. Therefore, range [j, i - 1] (i is n now) meet the condtion. So the size is i - j. If the condition is not met, it means it breaks in the middle of array. Range [j, i - 2] meet the condition, the size is i - 2 - j + 1 = i - j - 1
        if (meetCondition(state)) {
        ...
        // move the left index j as far as possible unitl the condition is met
        while (!meetCondition(state)) {
            // drop nums[j] from sliding window
            // Update the state
            j += 1;
...
**Solution:** We use the sum of the window elements as the state and check if it exceeds the target. First, we move the front pointer `i` until the sum meets the condition. Then, we verify that the end of the array hasn't been reached. Next, we shrink the window by moving the back pointer `j`. The window defined by `[j - 1, i - 1]` is the smallest one that meets the condition. We then compare this window with the final result and update it if the new window is smaller.
...
function minSubArrayLen(target: number, nums: number[]): number {
    ...
    let i = 0; // right index
    let j = 0; // left index
    let total = 0; // for window
    let ans = Infinity;
    const meetCondition = (total: number, target: number) => total >= target;
    while (i < n) {
        // move the right index i as far as possible until the condition is met
        while ((i < n) && !meetCondition(total, target)) {
            total += nums[i];
            i += 1;
        }
        if (!meetCondition(total, target)) {
            break;
        }
        // move the left index j as far as possible unitl the condition is not met
        while (meetCondition(total, target)) {
            total -= nums[j];
            j += 1;
        }
        // the range [j - 1, i - 1] meets the condition and [j, i - 1] does not meet.
...
function minWindow(s: string, t: string): string {
    const getIndex = (s: string, i: number): number => {
        ...
        throw new Error(`Incorrect character: ${s.charAt(i)}`);
    };
    ...
    const n = s.length;
    while (i < n) {
        while ((i < n) && !meetCondition(tFreq, sFreq)) {
            sFreq[getIndex(s, i)] += 1;
            i++;
        }
        ...
        while (meetCondition(tFreq, sFreq)) {
            sFreq[getIndex(s, j)] -= 1;
            j++;
        }
        // [j - 1, i - 1] meet the needs.
...
function minimumCardPickup(cards: number[]): number {
    ...
    let ans = Infinity;
    while (i < n) {
        while ((i < n) && !meetCondition(matchedCard)) {
        ...
        while (meetCondition(matchedCard)) {
            if (cards[j] === matchedCard as number) {
                matchedCard = undefined;
            } else {
                cardSet.delete(cards[j]);
            }
            j += 1;
        }
        ans = Math.min(ans, i - j + 1);
...
function lengthOfLongestSubstringKDistinct(s: string, k: number): number {
    ...
    while (i < n) {
        ...
        while ((i < n) && meetCondition(freq, k)) {
...
function totalFruit(fruits: number[]): number {
    ...
    while (i < n) {
        ...
        // freq contains the result from range: j to i - 1
        if (meetCondition(freq)) {
            ans = Math.max(ans, i - 1 - j + 1);
            break;
        } else {
            // range j to i - 2 meet the condition
            ans = Math.max(ans, i - 2 - j + 1);
        }
...
function findMaxConsecutiveOnes(nums: number[]): number {
    ...
    while (i < n) {
        // Move front as far as possible or front and end only conatin one zero.
        while (i < n && meetCondition(countZero)) {
            if (nums[i] === 0) {
                countZero += 1;
            }
            i += 1;
        }
        if (meetCondition(countZero)) {
            ans = Math.max(ans, i - j);
            break;
        } else {
            ans = Math.max(ans, i - j - 1)
