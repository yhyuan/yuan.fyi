...
class Solution:
    def numTrees(self, n: int) -> int:
        ...
        for i in range(1, n + 1):
            # Calculate the number of unique BSTs using each number as the root
            for j in range(1, i + 1):
                # The number of unique BSTs with j as the root = dp[j - 1] (left subtree) * dp[i - j] (right subtree)
                dp[i] += dp[j - 1] * dp[i - j]
