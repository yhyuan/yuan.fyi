...
**Initial Solution:** We create a proxy to wrap the access to the base object.
...
function produce(base, recipe) {
    ...
    const proxy = (function createProxy(value) {
        // Handle objects and arrays separately
        if (typeof value === 'object' && value !== null) {
            return new Proxy(value, {
                get(target, key) {
                    // Return a proxied version of nested objects or arrays
                    return createProxy(target[key]);
                },
                set(target, key, newValue) {
                    if (target[key] !== newValue) {
                        if (!isModified) {
                            // Clone the base state when the first modification is detected
                            isModified = true;
                        }
                        target[key] = newValue;
                    }
    ...
    // If changes were made, return the modified draft
    return proxy;
}
```

Checking Solution at https://bigfrontend.dev/problem/immerjs/discuss
